...
public class ExcelDataProvider implements Iterator<Object[]> {
	...
	public ExcelDataProvider(String methodname) {
		try {
            if(methodname.equals("walletpay")){
                columnNum=16;
            }
            if(methodname.equals("severalgoods")){
                columnNum=15;
            }
            if(methodname.equals("freightcoupon")){
                columnNum=16;
            }
            if(methodname.equals("freightwalletpay")){
                columnNum=18;
            }
			if(methodname.contains("payaccess")||methodname.contains("ioupay")){
				columnNum=5;
			}
			File directory = new File(".");
			this.book = Workbook.getWorkbook(new File(directory
					.getAbsolutePath() + "/src/TestData.xls"));
